...
static GmpResult getPacket(Gmp *ge, int *out1, int *out2, const char **error);
static GmpResult parsePacket(Gmp *ge, int *out1, int *out2, const char **error);
static GmpResult processCommand(Gmp *ge, Command command, int val, int *out1, int *out2, const char **error);
...
#define gmp_verified(ge) ((ge)->sizeVerified && (ge)->colorVerified && (ge)->handicapVerified && (ge)->rulesVerified)


Gmp *gmp_create(int inFile, int outFile)
{
    Gmp *ge = malloc(sizeof(Gmp));
    ge->inFile = inFile;
    ge->outFile = outFile;
...
GmpResult gmp_check(Gmp *ge, int gsleep, int *out1, int *out2, const char **error)
{
    fd_set readReady;
    ...
    if (gmp_verified(ge) && ge->earlyMovePresent) {
        ...
        *out2 = ge->earlyMoveY;
        ge->earlyMovePresent = 0;
        if (gmp_debug) {
            fprintf(stderr, "Goban GMP: Returning early move.\n");
        }
        return(gmp_move);
    }
    *out1 = 0;
    ...
    for (cNum = 0; cNum < count; ++cNum) {
        ...
        if (!ge->recvSoFar) {
            ...
            if ((c & 0xfc) == 0) {
                ...
                if (gmp_debug) {
                    fprintf(stderr, "Goban GMP: Received invalid packet.\n");
                }
            }
        }
        else {
...
static GmpResult parsePacket(Gmp *ge, int *out1, int *out2, const char **error)
{
    ...
    if (ge->recvData[2] & 0x08) {
        /* Not-understood command. */
        if (gmp_debug) {
            fprintf(stderr, "Goban GMP: Unknown command byte 0x%x received.\n", ge->recvData[2]);
        }
        return(gmp_nothing);
    }
    ...
    if (gmp_debug) {
        if (command == cmd_query) {
            if (val >= query_max) {
                if (gmp_debug)
                    fprintf(stderr, "Goban GMP: Read in command: %s unkown value %d\n", commandNames[command], val);
            }
            else {
                if (gmp_debug)
                    fprintf(stderr, "Goban GMP: Read in command: %s %s\n",
                            commandNames[command], queryNames[val]);
            }
        }
        else {
            if (gmp_debug)
                fprintf(stderr, "Goban GMP: Read in command: %s\n",
    ...
    if (!ge->waitingHighAck) {
        if ((command == cmd_ack) ||
            ...
            if (gmp_debug)
                fprintf(stderr, "Goban GMP: Unexpected ACK.\n");
            return(gmp_nothing);
        }
        else if (seq == ge->hisLastSeq) {
            /* Seen this one before. */
            if (gmp_debug)
                fprintf(stderr, "Goban GMP: Received repeated message.\n");
        ...
        if (command == cmd_ack) {
            ...
            if (gmp_debug)
                fprintf(stderr, "Goban GMP: his last seq = %d\n", seq);
            ge->hisLastSeq = seq;
            ge->waitingHighAck = 0;
            return(processCommand(ge, command, val, out1, out2, error));
        }
            ...
            if (gmp_debug)
                fprintf(stderr, "Goban GMP: Sending conflict.\n");
...
static GmpResult processCommand(Gmp *ge, Command command, int val, int *out1, int *out2, const char **error)
{
    ...
    switch(command) {
        ...
        case cmd_reset:
            /* New game. */
            if (gmp_debug)
                fprintf(stderr, "Goban GMP: Resetted. New game.\n");
            askQuery(ge);
            return(gmp_reset);
            break;
        case cmd_undo:
...
static void putCommand(Gmp *ge, Command cmd, int val)
{
    if (ge->waitingHighAck &&
        ...
        if (ge->sendsQueued < 1024) {
            ...
            if (gmp_debug)
                fprintf(stderr, "Goban GMP: Send buffer full. Catastrophic error.");
            exit(1);
        }
        return;
    }
    ...
    if (gmp_debug) {
        if (cmd == cmd_query)
            fprintf(stderr, "Goban GMP: Sending command: %s %s\n",
                    commandNames[cmd], queryNames[val]);
        else
            fprintf(stderr, "Goban GMP: Sending command: %s\n", commandNames[cmd]);
    }
...
static GmpResult respond(Gmp *ge, Query query)
{
    ...
    if (!wasVerified && gmp_verified(ge)) {
        if (gmp_debug)
            fprintf(stderr, "Goban GMP: New game ready.\n");
        return(gmp_newGame);
    }
    else {
        return(gmp_nothing);
    ...
    if (ge->waitingHighAck) {
        if (++ge->sendFailures > GMP_TIMEOUTSECS) {
            ...
            if (gmp_debug) {
                ...
                if (cmd == cmd_query) {
                    if (gmp_debug)
                        fprintf(stderr, "Goban GMP: Sending command: %s %s (retry)\n",
                                commandNames[cmd],
                                queryNames[ge->sendData[3] & 0x7f]);
                }
                else
                    if (gmp_debug)
                        fprintf(stderr, "Goban GMP: Sending command: %s (retry)\n",
                                commandNames[cmd]);
            }
            write(ge->outFile, ge->sendData, 4);
        }
...
void gmp_startGame(Gmp *ge, int size, int handicap, float komi, int chineseRules, int iAmWhite)
{
